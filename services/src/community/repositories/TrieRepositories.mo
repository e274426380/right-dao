...
module {
    ...
    public class TrieRepository<K, V>() {
        ...
        public func vals(db: TrieDB<K, V>) : [(K, V)]  {
            ...
            // let buffer = Buffer.Buffer<(K, V)>(0);

            // for (kv in Trie.iter<K, V>(db)) {
            //     buffer.add(kv);
            // };
            // buffer.toArray()
        };
    ...
    public func dbToCache<K, V>(db: TrieDB<K, V>, keyEq : (K, K) -> Bool, keyHash : K -> Hash.Hash) : TrieMap.TrieMap<K, V> {
        let arr = Trie.toArray<K, V, (K, V)>(db, func (k, v) : (K, V) { (k, v) });
        TrieMap.fromEntries(
            Array.vals<(K, V)>(arr),
            keyEq,
            keyHash
